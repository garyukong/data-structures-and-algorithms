...
# Create a set containing receiving area codes and mobile prefixes
receivingcodes = set()
for row in bangalorecalls:
    # Fixed lines
    fixedlineend = row[1].find(')')+1
    if fixedlineend > 0:
        receivingcodes.add(row[1][:fixedlineend])
    # Mobile numbers
    if row[1][0] in ['7','8','9']:
        receivingcodes.add(row[1][:4])

# Print outputs
print("The numbers called by people in Bangalore have codes:",*sorted(receivingcodes), sep = "\n")
